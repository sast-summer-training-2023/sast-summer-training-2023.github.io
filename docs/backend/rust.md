...
如果显示错误 `Commmand 'curl' not found` 的错误，并且运行的 WSL 是 Debian 或者 Ubuntu 发行版，请运行下面的命令：
...
VS Code 是后续课程中非常常见的一种开发工具，其支持的插件功能非常多样。而 Rust 相关的插件`rust-analyer`便是提高 Rust 开发体验中非常重要的一环，其提供的智能补全、语法检测与代码跳转功能对开发者来说极度舒适，也是我们推荐的对象。

打开 VS Code，在扩展市场中搜索`rust-analyer`，选择安装对应的插件。
...
但 Rust~~作为编程界的 Genshin Impact~~认为我全都要，即既要灵活性又要安全性。这时候，我们就需要对支持可变性的变量做出显式声明，将变量分为**可变变量**与**不可变变量**两种。
...
在开发中我们常会见到条件语句存在很多分支的情况，~~这时候我想大家不会喜欢狂写 10 个以上的`if else`语句~~，于是 Rust 引入了`match`关键字对应的匹配语句。
...
而 Rust 决定自主研发一款全新的~~不开放~~语言，因此提出了非常出名~~万恶之源~~的**所有权系统**。
...
而 Rust 的所有权机制通过编译器自动检查的方式，将检查时期放在了编译期，从而不会影响运行时性能，使得 Rust 拥有可以与 C++媲美的运行性能。另外智能的编译器以及完备的所有权规定也保证了自动检查不会出现问题，从而提高了安全性。~~当然你可以强制绕过编译器做一些不安全的行为，但是出事了请自己负责。~~
...
> 上图引用自 https://cn.bing.com/images/search?view=detailV2&ccid=n8OT0XBc&id=A4106DCFD449EE918AF4BD9058358E9E59683503&thid=OIP.n8OT0XBclpbJc0vKpLyEbQHaFY&mediaurl=https%3a%2f%2fimg-blog.csdnimg.cn%2f20190617110937653.png
...
~~肯定有人会说了，你这 Rust 变量这么容易暴毙，也太没用了吧。~~频繁的所有权转移确实会让人十分头疼，因此 Rust 引入了**引用机制**。**获取变量的引用，称为借用机制**。
...
~~Rust 传统默认不可变。~~按照我们之前不能修改变量的经历，我们也有理由猜测：我们不能直接修改引用内容。
...
我们的 Rust 为了安全起见，~~秉承解决不了就 ban 了的原则~~，设计了引用的规则，保证数据的安全性。这也是 Rust 正在逐渐成为系统编程方面的热门语言的重要原因。
...
3. 通过已有结构体实例进行实例化
   ...
   比如我们可以根据已有的 Genshin 实例 traveler 来构造新的实例 new_traverler。
...
你是否经历过：某一个物体存在着多种状态，然后你使用一个 int 来存储这个状态，其中数字 1 代表状态 1，2 代表状态 2，3 代表状态 3。。。然后写程序的时候，整个代码充满了意义不明的数字，而你~~作为懒人~~懒得写注释，导致代码可读性不是很好。

Rust 为了解决这个问题~~其实其他语言早就解决了，单纯是我们懒而已~~，推出了枚举类型。
...
* T、U 是定义给结构体的泛型参数
* V 是定义给方法的泛型参数
* 两者并不冲突，如果遇见了 V 和 W 不一样的情况会由编译器进行报错警告~~我们躺平就好~~
