...
<litepal>
    ...
    <list>
        ...
        <mapping class="com.example.MyModel" />
    </list>
</litepal>
```
...
public class MyModel extends LitePalSupport {
    private int id;
    private String name;
    private int age;
...
/*
    ...
    model1.setName("Clancy");
    model1.setAge(20);
    model1.save();

    MyModel model2 = new MyModel();
    model2.setName("elderly");
    model1.setAge(96);
    model2.save();

    List<MyModel> models = LitePal.where("name = ?", "elderly").find(MyModel.class);
    for (MyModel model : models) {
        // do something...
    }

    LitePal.updateAll(MyModel.class, "abmfy", "age < ?", "30");
    LitePal.deleteAll(MyModel.class, "age > ?", "80");
*/
```
